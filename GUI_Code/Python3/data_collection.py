...
========================
Data processing
This is how it is written in file:
...
* What data:
    ...
    inter trial stimulas interval - ((float - 4B)
    inter stimulas interval - ((float - 4B)
    missed trial interval - (float - 4B)
    response hold time - (int - 4B)
    stimulas duration time - (int - 4B)
...
class DataCollection:
    ...
    def write_data(self, config_data, rec_data):
        ...
        if not self.data_file:
            print(self.data_file, "Could not write data. 'self.data_file' is None (data_colletion: line 246)")
            return False
        ...
        with open(self.data_file, 'ab+') as f:
            ...
            for rec in rec_data:
                if len(rec) > 5:
                    finalSaveData = rec
                    # numData = rec[0:5] # Extract double and floating point data
                    strData = rec[5].encode() # Encode string to be able to write to binary
                    finalSaveData[5] = strData # Append data back together
                    # f.write(struct.pack(DATA_FORMAT, *rec))

                    try:
                        f.write(struct.pack(DATA_FORMAT, *finalSaveData)) # Save data
                    except Exception as randErr:
                        print('Failed to write data...')
                        print(finalSaveData)
                        raise

                else:
                    print('WARNING! Trial data was not complete. Not saving this trial''s data...')
    ...
    def raw_eye_task_data_limit_reached(self):
        # 2 options - Can check by
        #   list length or (I am assuming should not be a problem but please do check
        #   list size - else please use the list size check
    ...
    def dump_data_continually(self, data_recv_ts, task_data):
        ...
        # I am Sure if this is required. Basically anything
        #  that needs reinitialization can go here.
