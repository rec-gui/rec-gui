            ...
            while globls.is_program_running:
                ...
                if dt is not None:
                    ...
                    if float(cur_eye_time) != float(last_read_eye_time):
                        ...
                        # Add code to impliment automatic eye offset
                        if globls.auto_offset_flag:
                            # Check auto offset routine
                            if globls.auto_offset_dump_data:
                                globls.auto_offset_data_LEyeX = []
                                globls.auto_offset_data_LEyeY = []
                                globls.auto_offset_data_REyeX = []
                                globls.auto_offset_data_REyeY = []

                                # Reset all flags
                                globls.auto_offset_dump_data = 0
                                globls.auto_offset_stop_collect = 0
                                globls.auto_offset_start_collect = 0

                            else:
                                if globls.auto_offset_start_collect:
                                    # Make sure that data isn't longer than 1 second (1000 data points)
                                    #   If it is bigger than 1000, then it means that something went wrong so clear data
                                    if len(globls.auto_offset_data_LEyeX) > 1000:
                                        globls.auto_offset_dump_data = 1
                                        globls.auto_offset_stop_collect = 0
                                        globls.auto_offset_start_collect = 0
                                    else:
                                        # Keep collecting data
                                        globls.auto_offset_data_LEyeX.append(globls.eye_data[const.LEFT][const.X])
                                        globls.auto_offset_data_LEyeY.append(globls.eye_data[const.LEFT][const.Y])
                                        globls.auto_offset_data_REyeX.append(globls.eye_data[const.RIGHT][const.X])
                                        globls.auto_offset_data_REyeY.append(globls.eye_data[const.RIGHT][const.Y])

                                elif globls.auto_offset_stop_collect:
                                    # Stop collecting data and update the offset parameters
                                    if len(globls.auto_offset_data_LEyeX) > 150:
                                        meanLEyeX = np.mean(globls.auto_offset_data_LEyeX[-150:])
                                        meanLEyeY = np.mean(globls.auto_offset_data_LEyeY[-150:])
                                        meanREyeX = np.mean(globls.auto_offset_data_REyeX[-150:])
                                        meanREyeY = np.mean(globls.auto_offset_data_REyeY[-150:])
                                    else:
                                        print('Data not longer than 150 data points.')
                                        meanLEyeX = np.mean(globls.auto_offset_data_LEyeX)
                                        meanLEyeY = np.mean(globls.auto_offset_data_LEyeY)
                                        meanREyeX = np.mean(globls.auto_offset_data_REyeX)
                                        meanREyeY = np.mean(globls.auto_offset_data_REyeY)

                                    # Update offset values and save in globls
                                    globls.auto_offset_tmpOffset_LEyeX = globls.offset_gain_config[ConfigLabels.LEFT_EYE_OFFSET_X] - np.round(meanLEyeX, 1)
                                    globls.auto_offset_tmpOffset_LEyeY = globls.offset_gain_config[ConfigLabels.LEFT_EYE_OFFSET_Y] - np.round(meanLEyeY, 1)
                                    globls.auto_offset_tmpOffset_REyeX = globls.offset_gain_config[ConfigLabels.RIGHT_EYE_OFFSET_X] - np.round(meanREyeX, 1)
                                    globls.auto_offset_tmpOffset_REyeY = globls.offset_gain_config[ConfigLabels.RIGHT_EYE_OFFSET_Y] - np.round(meanREyeY, 1)

                                    # print('New offset values: ', globls.auto_offset_tmpOffset_LEyeX,
                                    #       globls.auto_offset_tmpOffset_LEyeX, globls.auto_offset_tmpOffset_LEyeX,
                                    #       globls.auto_offset_tmpOffset_LEyeX)

                                    # Clear data and reset flags
                                    globls.auto_offset_dump_data = 1
                                    globls.auto_offset_stop_collect = 0
                                    globls.auto_offset_start_collect = 0
