...
class Gui:
    def __init__(self):
        ...
        self.auto_offset_button = tk.IntVar()
    ...
    def read_eye_window_configuration(self):
        try:
            ...
            # Check if GUI needs to update the offset values from the Global Parameters
            if globls.auto_offset_update_flag == 1 & globls.auto_offset_update_primer == 1 & globls.auto_offset_flag == 1:
                # Update GUI offset values
                self.h_left_offset.set(str(np.round(globls.auto_offset_tmpOffset_LEyeX, 1)))
                self.v_left_offset.set(str(np.round(globls.auto_offset_tmpOffset_LEyeY, 1)))
                self.h_right_offset.set(str(np.round(globls.auto_offset_tmpOffset_REyeX, 1)))
                self.v_right_offset.set(str(np.round(globls.auto_offset_tmpOffset_REyeY, 1)))

                # Reset globls.auto_offset_update_flag and primer
                globls.auto_offset_update_flag = 0
                globls.auto_offset_update_primer = 0

            # Read offset gain and send to ripple if updated
            if len(self.pupil_size[const.LEFT].get()):
                globls.pupil_size['left'] = int(self.pupil_size[const.LEFT].get())
            ...
            if len(self.h_left_offset.get()):
                # Check if value has changed. If so, send to ripple
                if globls.XIPPPY_CONNECTION:
                    if globls.offset_gain_config['left_offset_x'] != float(self.h_left_offset.get()):
                        # Calculate value to send based on equation: identifier + 100*value
                        tmpVal = int(globls.XIPPPY_XOFFSET_LEFT) + int(100 * float(self.h_left_offset.get()))
                        xp.digout([globls.XIPPPY_PARALLELOUT_IDX], [int(globls.XIPPPY_XOFFSET_LEFT)])
                        xp.digout([globls.XIPPPY_PARALLELOUT_IDX], [int(tmpVal)])
                globls.offset_gain_config['left_offset_x'] = float(self.h_left_offset.get())

            if len(self.h_left_gain.get()):
                # Check if value has changed. If so, send to ripple
                if globls.XIPPPY_CONNECTION:
                    if globls.offset_gain_config['left_gain_x'] != float(self.h_left_gain.get()):
                        # Calculate value to send based on equation: identifier + 100*value
                        tmpVal = int(globls.XIPPPY_XGAIN_LEFT) + int(100 * float(self.h_left_gain.get()))
                        xp.digout([globls.XIPPPY_PARALLELOUT_IDX], [int(globls.XIPPPY_XGAIN_LEFT)])
                        xp.digout([globls.XIPPPY_PARALLELOUT_IDX], [int(tmpVal)])
                globls.offset_gain_config['left_gain_x'] = float(self.h_left_gain.get())
            ...
            if len(self.v_left_offset.get()):
                # Check if value has changed. If so, send to ripple
                if globls.XIPPPY_CONNECTION:
                    if globls.offset_gain_config['left_offset_y'] != float(self.v_left_offset.get()):
                        # Calculate value to send based on equation: identifier + 100*value
                        tmpVal = int(globls.XIPPPY_YOFFSET_LEFT) + int(100 * float(self.v_left_offset.get()))
                        xp.digout([globls.XIPPPY_PARALLELOUT_IDX], [int(globls.XIPPPY_YOFFSET_LEFT)])
                        xp.digout([globls.XIPPPY_PARALLELOUT_IDX], [int(tmpVal)])
                globls.offset_gain_config['left_offset_y'] = float(self.v_left_offset.get())
            ...
            if len(self.v_left_gain.get()):
                # Check if value has changed. If so, send to ripple
                if globls.XIPPPY_CONNECTION:
                    if globls.offset_gain_config['left_gain_y'] != float(self.v_left_gain.get()):
                        # Calculate value to send based on equation: identifier + 100*value
                        tmpVal = int(globls.XIPPPY_YGAIN_LEFT) + int(100 * float(self.v_left_gain.get()))
                        xp.digout([globls.XIPPPY_PARALLELOUT_IDX], [int(globls.XIPPPY_YGAIN_LEFT)])
                        xp.digout([globls.XIPPPY_PARALLELOUT_IDX], [int(tmpVal)])
                globls.offset_gain_config['left_gain_y'] = float(self.v_left_gain.get())
            ...
            if len(self.h_right_offset.get()):
                # Check if value has changed. If so, send to ripple
                if globls.XIPPPY_CONNECTION:
                    if globls.offset_gain_config['right_offset_x'] != float(self.h_right_offset.get()):
                        # Calculate value to send based on equation: identifier + 100*value
                        tmpVal = int(globls.XIPPPY_XOFFSET_RIGHT) + int(100 * float(self.h_right_offset.get()))
                        xp.digout([globls.XIPPPY_PARALLELOUT_IDX], [int(globls.XIPPPY_XOFFSET_RIGHT)])
                        xp.digout([globls.XIPPPY_PARALLELOUT_IDX], [int(tmpVal)])
                globls.offset_gain_config['right_offset_x'] = float(self.h_right_offset.get())
            ...
            if len(self.h_right_gain.get()):
                # Check if value has changed. If so, send to ripple
                if globls.XIPPPY_CONNECTION:
                    if globls.offset_gain_config['right_gain_x'] != float(self.h_right_gain.get()):
                        # Calculate value to send based on equation: identifier + 100*value
                        tmpVal = int(globls.XIPPPY_XGAIN_RIGHT) + int(100 * float(self.h_right_gain.get()))
                        xp.digout([globls.XIPPPY_PARALLELOUT_IDX], [int(globls.XIPPPY_XGAIN_RIGHT)])
                        xp.digout([globls.XIPPPY_PARALLELOUT_IDX], [int(tmpVal)])
                globls.offset_gain_config['right_gain_x'] = float(self.h_right_gain.get())
            ...
            if len(self.v_right_offset.get()):
                # Check if value has changed. If so, send to ripple
                if globls.XIPPPY_CONNECTION:
                    if globls.offset_gain_config['right_offset_y'] != float(self.v_right_offset.get()):
                        # Calculate value to send based on equation: identifier + 100*value
                        tmpVal = int(globls.XIPPPY_YOFFSET_RIGHT) + int(100 * float(self.v_right_offset.get()))
                        xp.digout([globls.XIPPPY_PARALLELOUT_IDX], [int(globls.XIPPPY_YOFFSET_RIGHT)])
                        xp.digout([globls.XIPPPY_PARALLELOUT_IDX], [int(tmpVal)])
                globls.offset_gain_config['right_offset_y'] = float(self.v_right_offset.get())
            ...
            if len(self.v_right_gain.get()):
                # Check if value has changed. If so, send to ripple
                if globls.XIPPPY_CONNECTION:
                    if globls.offset_gain_config['right_gain_y'] != float(self.v_right_gain.get()):
                        # Calculate value to send based on equation: identifier + 100*value
                        tmpVal = int(globls.XIPPPY_YGAIN_RIGHT) + int(100 * float(self.v_right_gain.get()))
                        xp.digout([globls.XIPPPY_PARALLELOUT_IDX], [int(globls.XIPPPY_YGAIN_RIGHT)])
                        xp.digout([globls.XIPPPY_PARALLELOUT_IDX], [int(tmpVal)])
                globls.offset_gain_config['right_gain_y'] = float(self.v_right_gain.get())
    ...
    def draw_eye_and_window_config(self):
        ...
        # Auto offset
        eye_win_widget['auto_offset']['variable'] = self.auto_offset_button
        eye_win_widget['auto_offset']['command'] = self.process_left_right_checkbutton
        WidgetApi.draw_check_button(eye_win_config, eye_win_widget['auto_offset'])
    ...
    def update_default_eye_configuration(self, init=False):
        ...
        # Set initial value for auto offset
        self.auto_offset_button.set(0)
    ...
    def process_left_right_checkbutton(self):
        ...
        globls.auto_offset_flag = self.auto_offset_button.get()
    ...
    def process_load_save_task_file(self, operation):
        try:
            ...
            if operation == 'Load':
                ...
                od = collections.OrderedDict(self.displayable_widget_config.items())
    ...
    def on_exit(self):
        ...
        if messagebox.askyesno("Close GUI?",
    ...
    def update_displayable_configuration_dict(self, event=None):
        ...
        if self.displayable_widget_config.get(event.widget, None):
            ...
            if len(command_word):
                ...
                for k, v in self.displayable_configuration_dict.items():
