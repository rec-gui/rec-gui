...
class ValidateVergenceVersion():
    ...
    def eye_in_out(self):
        ...
        if globls.latched_eye:
            # The GUI caught the eye moving out, but MATLAB has not indicated that it received the message and ended the latched eye method
            if globls.caught:
                data = '{} {}/'.format(
                            const.LATCHED_EYE_STATUS, 1
                            )
                util.send_data(globls.arbitrator_sock_eye, globls.arbitrator_sock_addr_eye, data)
            elif left_eye_in_out == 0:
                data = '{} {}/'.format(
                            const.LATCHED_EYE_STATUS, 1
                            )
                util.send_data(globls.arbitrator_sock_eye, globls.arbitrator_sock_addr_eye, data)
                globls.caught = 1
                globls.dump_log('Ending latched eye request due to violation')
            elif right_eye_in_out == 0:
                data = '{} {}/'.format(
                            const.LATCHED_EYE_STATUS, 1
                            )
                util.send_data(globls.arbitrator_sock_eye, globls.arbitrator_sock_addr_eye, data)
    ...
    def vergence_error(self, target_pt_data):
        ...
        #globls.dump_log('Correct Vergence {}'.format(perfect_delta_x))
        #globls.dump_log('Eye Left x {}'.format(left_eye_x))
        #globls.dump_log('Eye Right x {}'.format(right_eye_x))
        #globls.dump_log('vergence_magnitude_error {}'.format(vergence_magnitude_error))
